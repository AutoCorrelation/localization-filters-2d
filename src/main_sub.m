...
fprintf('\n=== RMSE Comparison ===\n');
fprintf('Noise Level | baseline | LinearKF    | LinearPF    | LinearKF_DecayQ | NonLinearPF\n');
fprintf('--------------------------------------------------------------\n');
numNoise = numel(config.noiseVariance);
for i = 1:numNoise
    fprintf('%.0e   | %.4f | %.4f | %.4f | %.4f     | %.4f\n', ...
        config.noiseVariance(i), baseline_RMSE(i), lkf_RMSE(i), lpf_RMSE(i), lkf_decayQ_RMSE(i), nl_pf_RMSE(i));
end
fprintf('\n');

plotRMSEComparison(config.noiseVariance, baseline_RMSE, lkf_RMSE, lpf_RMSE, lkf_decayQ_RMSE, nl_pf_RMSE);