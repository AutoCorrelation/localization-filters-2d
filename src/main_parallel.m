...
parfor countNoise = 1:numNoise
    pf_data_noise = struct();
    pf_data_noise.estimatedPos = zeros(2, numPoints, pfIterations);
    ...
    for countIter = 1:pfIterations
        ...
        for countPoint = 2:numPoints
            ...
            if countPoint < 3
                pf_data_noise.estimatedPos(:, countPoint-1, countIter) = toaPos(:, countIter, countPoint-1, countNoise);
                pf_data_noise.estimatedPos(:, countPoint, countIter) = toaPos(:, countIter, countPoint, countNoise);
                ...
                [particles_res, weights_upd] = pf.multinomial_resampling(particles_pred, weights_upd);
                ...
                pf_data_noise.estimatedPos(:, countPoint, countIter) = est;
            end
        end
    end
    ...
    for countIter = 1:pfIterations
        for countPoint = 1:numPoints
            rmse_temp = rmse_temp + norm(pf_data_noise.estimatedPos(:, countPoint, countIter) - [countPoint; countPoint]);
        end
    end
    pf_RMSE(countNoise) = rmse_temp / (numPoints * pfIterations);
    
    pf_data_all{countNoise} = pf_data_noise;
end
